...
export default function MDStudyForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // {{message}} variable.
      const messageBody = [
        'MEDICAL DEVICE SUMMATIVE STUDY — PHYSICIAN APPLICATION',
        '',
        `Name: ${data.name}`,
        `Email: ${data.email}`,
        `Phone: ${data.phone}`,
      ...
      await emailjs.send(
        ...
        {
          from_name: `${data.name} (Physician Study)`,
      ...
      console.error('MD study form submission failed', err);
